...
load D:\highimbalance\yeast\data_5rand\train5.txt;
...
k0=1;
c=size(ntr,1);
fid=fopen('D:\highimbalance\yeast\inv_dc1\result\basicdata_train5.txt','w');
...
for r=1:c-1,
    for t=r+1:c,
        ...
        d=d0;
        a0=[];
        a0(:,2:dim+1)=train5(ntr(r,1):ntr(r,2),1:dim);
        ...
        a0(p0+1:p0+q0,2:dim+1)=train5(ntr(t,1):ntr(t,2),1:dim);        
        if q0==1
...
for r=1:c-1,
    for t=r+1:c,
        ...
        p0=ntr(r,3);
        q0=ntr(t,3);
        a=train5(ntr(r,1):ntr(r,2),1:dim);
        a(p0+1:p0+q0,:)=train5(ntr(t,1):ntr(t,2),1:dim);
...
% AUC
        ...
        area_train5(r,t)=0;
        for i=2:p0+q0+1,
            area_train5(r,t)=area_train5(r,t)+b(i,2)*(b(i,1)-b(i-1,1));
        end
        area_train5(r,t);
...
%        fprintf(fid,'(r,t)=%d\n',r,t);
        ...
        err_train5(r,t-1)=100*(1-(numtr0(1)+numtr0(2))/(p0+q0));
        err_train5(r,c+t-2)=100*((p0-numtr0(1))/p0+(q0-numtr0(2))/q0)/2;
        err_train5(r,2*c+t-3)=100*sqrt(((p0-numtr0(1))/p0)*((q0-numtr0(2))/q0));
        err_train5(r,3*c+t-4)=100*area_train5(r,t);
        err_train5(r,4*c+t-5)=(err_train5(r,t-1)+err_train5(r,c+t-2)+err_train5(r,2*c+t-3)+area_train5(r,t)*100)/4;
    end
end

err_train5
fid=fopen('D:\highimbalance\yeast\inv_dc1\result\err_train5.txt','w');
...
% Total, average, geometric accuracy; AUC:
% For the test5 set.
load D:\highimbalance\yeast\data_5rand\test5.txt;
load D:\highimbalance\yeast\nte.txt;
load D:\highimbalance\yeast\inv_dc1\result\w0.txt;
load D:\highimbalance\yeast\inv_dc1\result\w.txt;
load D:\highimbalance\yeast\inv_dc1\result\w1.txt;
...
for r=1:c-1,
    for t=r+1:c,
        ...
        p0=nte(r,3);
        q0=nte(t,3);
        a=test5(nte(r,1):nte(r,2),1:dim);
        a(p0+1:p0+q0,:)=test5(nte(t,1):nte(t,2),1:dim);
        re0=a*(w0(k,:))'+ones(p0+q0,1)*theta0(k);
        kk=sum(sign0(1:k));
        if sign0(k)==7
           re=a*(w(kk-6:kk,:))'+ones(p0+q0,1)*theta(kk-6:kk)';
...
% AUC
        ...
        area_test5(r,t)=0;
        for i=2:p0+q0+1,
            area_test5(r,t)=area_test5(r,t)+b(i,2)*(b(i,1)-b(i-1,1));
        end
        area_test5(r,t);
...
%        fprintf(fid,'(r,t)=%d\n',r,t);
        ...
        err_test5(r,t-1)=100*(1-(numtr0(1)+numtr0(2))/(p0+q0));
        err_test5(r,c+t-2)=100*((p0-numtr0(1))/p0+(q0-numtr0(2))/q0)/2;
        err_test5(r,2*c+t-3)=100*sqrt(((p0-numtr0(1))/p0)*((q0-numtr0(2))/q0));
        err_test5(r,3*c+t-4)=100*area_test5(r,t);
        err_test5(r,4*c+t-5)=(err_test5(r,t-1)+err_test5(r,c+t-2)+err_test5(r,2*c+t-3)+area_test5(r,t)*100)/4;
    end
end

err_test5
fid=fopen('D:\highimbalance\yeast\inv_dc1\result\err_test5.txt','w');
    fprintf(fid,'%g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g\n',err_test5');
fclose(fid);
...
% Majority vote for the train5ing set
...
for i=1:n0,
    re0=train5(i,1:dim)*w0'+theta0';
    re1=train5(i,1:dim)*w'+theta';
    re2=train5(i,1:dim)*w1'+theta1';
    re3=train5(i,1:dim)*w2'+theta2';
    n00=0;
    for j=1:c-1,
        for k=j+1:c,
            n00=n00+1;
...
fid=fopen('D:\highimbalance\yeast\inv_dc1\result\Number_error_train5.txt','w');
    fprintf(fid,'\nFor the train5 set\n');
    fprintf(fid,'\n%g %g %g %g %g %g %g %g %g %g\n',nerr');
    fprintf(fid,'\n%g %g %g %g %g %g %g %g %g %g\n',nerr1');
    fprintf(fid,'\n%g\n',sum(nerr'));
...
%clear all;
%t0=clock;
% Majority vote for the test5 set
...
for i=1:n0,
    re0=test5(i,1:dim)*w0'+theta0';
    re1=test5(i,1:dim)*w'+theta';
    re2=test5(i,1:dim)*w1'+theta1';
    re3=test5(i,1:dim)*w2'+theta2';
    n00=0;
    for j=1:c-1,
        for k=j+1:c,
            n00=n00+1;
...
fid=fopen('D:\highimbalance\yeast\inv_dc1\result\Number_error_test5.txt','w');
    fprintf(fid,'\nFor the test5 set\n');
    fprintf(fid,'\n%g %g %g %g %g %g %g %g %g %g\n',nerr');
    fprintf(fid,'\n%g %g %g %g %g %g %g %g %g %g\n',nerr1');
    fprintf(fid,'\n%g\n',sum(nerr'));
